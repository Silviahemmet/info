(function () {
  ...
  var historyItem = {
    ...
    image: '',
    imageCredit: 'Arkivbild: Finna'
  };
  var history75Item = {
    title: 'Ett arkivfynd från Finland på dagens datum år 1951.',
    date: 'Finland 1951',
    link: 'https://www.finna.fi',
    image: '',
    imageCredit: 'Arkivbild: Finna'
  };
  var historyPhotoStep = 0;
  ...
  function parseNameDay(html) {
    ...
    if (match) {
      currentNameDay = match[1];
      updateTopNameDay();
    }
  }

  function updateTopNameDay() {
    var row = document.querySelector ? document.querySelector('.day-info') : null;
    setText('top-name', currentNameDay);
    if (!row) { return; }
    row.className = row.className.replace(/\s*long-name-day/g, '');
    if (String(currentNameDay).length > 18) { row.className += ' long-name-day'; }
  }
  ...
  function setWeatherArt(code) {
    ...
    if (!panel) { return; }
    if (art) {
      if (code === 1 || code === 2) { art.src = 'forecast-partly-transparent-v2.png?v=1'; }
      else if (code === 3) { art.src = 'forecast-cloud-transparent-v5.png?v=1'; }
  ...
  function forecastImage(code) {
    ...
    if (code === 1 || code === 2) { return 'forecast-partly-transparent-v2.png?v=1'; }
    if (isRain(code)) { return 'forecast-cloud-transparent-v5.png?v=1'; }
    return 'forecast-cloud-transparent-v5.png?v=1';
  }
  ...
  function setHistoryFallback() {
    ...
    var existingImage = historyItem.image || '';
    var existingCredit = historyItem.imageCredit || 'Arkivbild: Finna';
    var exactFacts = {
      '1976-07-30': {
    ...
    historyItem.isExact = !!exactFacts[key];
    historyItem.date = day + '.' + month + '.' + targetYear;
    historyItem.image = existingImage;
    historyItem.imageCredit = existingCredit;
    refreshHistoryPhoto();
  }

  function setHistory75Fallback() {
    var now = new Date();
    var targetYear = now.getFullYear() - 75;
    var month = now.getMonth() + 1;
    var day = now.getDate();
    var key = targetYear + '-' + pad(month) + '-' + pad(day);
    var existingImage = history75Item.image || '';
    var existingCredit = history75Item.imageCredit || 'Arkivbild: Finna';
    var exactFacts = {
      '1951-07-30': {
        title: 'För exakt 75 år sedan spelades skivan “Unikuva” in i Helsingfors med Metro-tytöt, Marianne och Kullervo Linnas orkester.',
        link: 'https://www.finna.fi/Record/fikka.4699339'
      }
    };
    history75Item = exactFacts[key] || {
      title: 'Ett arkivfynd från Finland på dagens datum år ' + targetYear + '.',
      link: 'https://www.finna.fi'
    };
    history75Item.isExact = !!exactFacts[key];
    history75Item.date = day + '.' + month + '.' + targetYear;
    history75Item.image = existingImage;
    history75Item.imageCredit = existingCredit;
    refreshHistoryPhoto();
  }

  function renderHistoryPhoto(item, heading) {
    var image = byId('history-photo-image');
    setText('history-photo-heading-text', heading);
    setText('history-photo-date', item.date || '');
    setText('history-photo-text', item.title || '');
    setText('history-photo-credit', item.imageCredit || 'Arkivbild: Finna');
    if (!image) { return; }
  ...
  function refreshHistoryPhoto() {
    if (historyPhotoStep === 1) {
      renderHistoryPhoto(historyItem, 'Finland för 50 år sedan');
    } else if (historyPhotoStep === 2) {
      renderHistoryPhoto(history75Item, 'Finland för 75 år sedan');
    }
  }

  function toggleHistoryPhoto() {
    var photo = document.querySelector ? document.querySelector('.photo') : null;
    if (!photo) { return; }
    historyPhotoStep = (historyPhotoStep + 1) % 3;
    if (historyPhotoStep === 0) {
      photo.className = photo.className.replace(/\s*show-history/g, '');
      return;
    }
    refreshHistoryPhoto();
    if (photo.className.indexOf('show-history') === -1) { photo.className += ' show-history'; }
  }

  function loadArchivePhoto(item, targetYear) {
    var now = new Date();
    var filters = [
      'online_boolean:"1"',
      'format:"0/Image/"',
      'search_daterange_mv:"[' + targetYear + ' TO ' + targetYear + ']"',
      '~usage_rights_ext_str_mv:"0/A Free/"'
    ];
    var url = 'https://api.finna.fi/v1/search?lookfor=' + encodeURIComponent(String(targetYear)) + '&type=AllFields&limit=100&lng=sv';
    var i;
    for (i = 0; i < filters.length; i += 1) {
      url += '&filter%5B%5D=' + encodeURIComponent(filters[i]);
    }
    url += '&field%5B%5D=id&field%5B%5D=title&field%5B%5D=year&field%5B%5D=images';
    request(url, function (response) {
      var data;
      var records;
      var suitable = [];
      ...
      var imagePath;
      var choice;
      try { data = JSON.parse(response); records = data.records || []; } catch (ignore) { return; }
      for (i = 0; i < records.length; i += 1) {
        record = records[i];
        if (record.images && record.images.length && String(record.year || '').indexOf(String(targetYear)) !== -1) {
          suitable.push(record);
        }
      }
      if (!suitable.length) { return; }
      choice = (now.getMonth() * 31 + now.getDate()) % suitable.length;
      record = suitable[choice];
      imagePath = String(record.images[0] || '');
      if (!imagePath) { return; }
      item.image = imagePath.indexOf('http') === 0 ? imagePath : 'https://api.finna.fi' + imagePath;
      item.imageCredit = 'Arkivbild från ' + targetYear + ' • Finna';
      refreshHistoryPhoto();
    }, function () {});
  }

  function loadHistoryPhoto() {
    loadArchivePhoto(historyItem, new Date().getFullYear() - 50);
  }

  function loadHistory75Photo() {
    loadArchivePhoto(history75Item, new Date().getFullYear() - 75);
  }
  ...
  function loadHistory75() {
    var now = new Date();
    var targetYear = now.getFullYear() - 75;
    var searchDate = now.getDate() + '.' + (now.getMonth() + 1) + '.' + targetYear;
    var url;
    setHistory75Fallback();
    if (history75Item.isExact) { return; }
    url = 'https://api.finna.fi/v1/search?lookfor=' + encodeURIComponent('"' + searchDate + '"') + '&type=AllFields&limit=20&lng=sv';
    url += '&field%5B%5D=id&field%5B%5D=title&field%5B%5D=year';
    request(url, function (response) {
      var data;
      var records;
      var i;
      ...
      for (i = 0; i < records.length; i += 1) {
        record = records[i];
        title = String(record.title || '').replace(/\s+/g, ' ').replace(/^\s+|\s+$/g, '');
        if (title && String(record.year || '').indexOf(String(targetYear)) !== -1) {
          history75Item.title = 'Arkivfynd från dagens datum: “' + title + '” dokumenterades i Finland för 75 år sedan.';
          history75Item.link = record.id ? 'https://www.finna.fi/Record/' + encodeURIComponent(record.id) : 'https://www.finna.fi';
          refreshHistoryPhoto();
          return;
        }
      }
  ...
  window.setInterval(loadHistory75, 21600000);
  window.setInterval(loadHistoryPhoto, 21600000);
  window.setInterval(loadHistory75Photo, 21600000);
  window.setInterval(toggleHistoryPhoto, 30000);
